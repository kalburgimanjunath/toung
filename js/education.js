	function createEducationDisplayTemplate() {
		$.template( "edudisp", [
			...
			"<div class='disp'><ul class='edu-data'>",
			"    <li><h2>${institution}</h2></li>",
			"    <li><h3>${credential}</h3></li>",
			"    <li><h4>${fieldofstudy}</h4></li>",
			"    <li><h5>${startmonth}, ${startyear} - ${endmonth}, ${endyear}</h5></li>",
			"    <li><i>${activities}</h5></li>",
			"    <li><li class='timestamp'>Updated ${updatedago}</li>",
			"</ul></div>",
			"<div class='actions' style='display:inline-block'>",
			"<a class='actionlink rounded' id='edu-modify-${posnum}' data='${posnum}' title='Make changes' href=''>Change</a>",
			"<a class='actionlink rounded' id='edu-delete-${posnum}' data='${posnum}' title='Remove this forever' href=''>Remove</a>",
			"</div>",
...
*/
	...
	function populateEducationInfo(edudiv, edulist) {
        ...
        edudiv.empty();
        edudiv.append("<h2 class='rounded'>Education</h2>");
        edudiv.append("<div id='edu-norecords' class='norecords' style='display:none'>You do not have any information at the moment. Please click on the button below to add.</div>");
        edudiv.append("<div id='addeducation'><input type='submit' value='Add Education' title='Add more education' name='Add Education' id='addnewedu' class='submitbutton'></div>");
