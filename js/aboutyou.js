	...
	function loadAndShowAboutyou(abtdiv) {
		...
		$.ajax({
		     ...
		     success: function(response, status){
					if (response.error) {
						...
						populateAboutYouInfo(abtdiv, response.data /* abtlist */);
	                }
             },
...
*/
	...
	function populateAboutYouInfo(abtdiv, abtlist) {

        abtdiv.empty();
        abtdiv.append("<h2 class='rounded'>About You</h2>");
        abtdiv.append("<div id='edu-norecords' class='norecords' style='display:none'>You do not have any information at the moment. Please click on the button below to add.</div>");
       
		if ((abtlist) && (abtlist.length > 0)) {
    		$("#edu-norecords").hide();
	        for (i=0; i < abtlist.length; i++) {
				createEducationInfo(abtdiv, abtlist[i]);                       
        	}
    	} else {
        	$("#edu-norecords").show();
    	}
